      ...
      <nav className="glass-deep rounded-[28px] border border-[rgba(196,115,122,0.12)] px-3 py-3 shadow-[var(--shadow-lift-sm)]">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[14px] font-semibold tracking-[-0.02em] text-[var(--ink)]">交个代</p>
              <p className="text-[10px] font-medium uppercase tracking-[0.06em] text-[var(--ink-faint)]">Mobile Stack Mode</p>
            </div>
          </div>
        ...
        {navigationItems.map((item) => (
          <button
            ...
            className={`nav-item mb-2 flex h-12 w-12 items-center justify-center rounded-[18px] ${
              item.active ? 'bg-[var(--petal-light)] text-[var(--petal)]' : 'text-[var(--ink-faint)] hover:bg-[var(--petal-xlight)] hover:text-[var(--petal)]'
            }`}
            aria-label={item.label}
          >
            <HomeIcon icon={item.icon} className="h-5 w-5" />
...
export function MainTaskWorkspace({ mode }: { mode: WorkspaceMode }) {
  ...
  return (
    <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      <header className={`flex flex-col gap-4 px-5 pb-4 pt-6 ${isDesktop ? 'md:px-8 md:pt-8 lg:flex-shrink-0' : ''}`}>
        <div className={`flex flex-col gap-4 ${isDesktop ? 'lg:flex-row lg:items-start lg:justify-between' : 'md:flex-row md:items-start md:justify-between'}`}>
          <div>
            <p className="mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--ink-faint)]">
              <SunIcon className="h-3 w-3" />
              周二，5 月 8 日
            </p>
            <h2 className={`font-light leading-[1.1] tracking-[-0.03em] text-[var(--ink)] ${isMobile ? 'text-[28px]' : isDesktop ? 'text-[32px] md:text-[36px]' : 'text-[34px]'}`}>
              早安。<span className="font-medium text-[var(--petal)]">『{userName}』</span>
            </h2>
            <p className={`mt-1.5 text-[14px] leading-6 text-[var(--ink-muted)] ${isMobile ? 'max-w-[280px]' : 'max-w-3xl'}`}>
      ...
      <div className={`grid gap-3 px-5 pb-5 ${isMobile ? 'grid-cols-2' : isDesktop ? 'sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_192px] md:px-8 lg:flex-shrink-0' : 'grid-cols-[repeat(3,minmax(0,1fr))_180px]'}`}>
        <StatTile icon={<CheckCircleIcon className="h-4 w-4 text-[var(--petal)]" />} iconClassName="bg-[var(--petal-light)]" value="3" suffix="/ 7" label="已完成任务" />
        <StatTile icon={<SparkIcon className="h-4 w-4 text-[var(--teal)]" />} iconClassName="bg-[var(--teal-xlight)]" value="2" suffix="项" label="关键产出" />
        {!isMobile ? (
          <StatTile icon={<TimerIcon className="h-4 w-4 text-[#c48240]" />} iconClassName="bg-[rgba(232,168,124,0.15)]" value="2小时" suffix="14分" label="深度专注" />
        ) : null}
        <article className="stat-card glass flex items-center gap-4 rounded-xl border border-[rgba(196,115,122,0.1)] px-4 py-3">
          <div className="relative h-12 w-12 flex-shrink-0">
            <svg viewBox="0 0 48 48" className="h-12 w-12">
              ...
              <circle cx="24" cy="24" r="19" fill="none" stroke="var(--petal)" strokeWidth="4" strokeLinecap="round" strokeDasharray="119.38" strokeDashoffset="40" transform="rotate(-90 24 24)" />
            </svg>
        ...
        <div className="glass rounded-xl border border-[rgba(196,115,122,0.1)] p-4 shadow-[var(--shadow-lift-sm)]">
          <div className={`flex flex-col gap-4 ${isDesktop ? 'lg:flex-row lg:items-start lg:justify-between' : 'md:flex-row md:items-start md:justify-between'}`}>
            <div>
              ...
              <h3 className="text-[20px] font-medium leading-7 tracking-[-0.015em] text-[var(--ink)]">日报草稿会基于任务状态、结果说明和未完成原因自动生成。</h3>
              <p className="mt-1.5 max-w-3xl text-[14px] leading-6 text-[var(--ink-muted)]">默认结构包含今日完成、关键产出、未完成事项及原因、明日计划，并支持查看引用来源任务。</p>
            </div>
...
export function TabletAiOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      ...
      />
      <section className={`glass-deep fixed bottom-28 right-6 z-50 w-[360px] rounded-[30px] border border-[rgba(196,115,122,0.14)] p-4 shadow-[0_8px_40px_rgba(28,22,20,0.08),0_16px_56px_rgba(196,115,122,0.12)] transition-all ${open ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-5 opacity-0'}`}>
    ...
    <section className="relative z-20 min-h-screen px-4 pb-6 pt-4">
      <div className="glass-deep flex min-h-[calc(100vh-2rem)] flex-col rounded-[32px] border border-[rgba(196,115,122,0.14)] shadow-[0_8px_40px_rgba(28,22,20,0.08),0_16px_56px_rgba(196,115,122,0.12)]">
        <div className="flex items-center justify-between border-b border-[rgba(196,115,122,0.1)] px-4 py-4">
          <div className="flex items-center gap-3">
            <button className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[rgba(28,22,20,0.04)] text-[var(--ink-faint)]" onClick={onBack}>
            ...
            <div>
              <p className="text-[14px] font-semibold tracking-[-0.01em] text-[var(--ink)]">代办官</p>
              <p className="text-[11px] text-[var(--teal)]">独立 AI 聊天界面</p>
            </div>
          </div>
          <button className="rounded-full bg-[var(--petal-xlight)] px-3 py-1 text-[11px] font-semibold text-[var(--petal)]">今天</button>
        </div>
...
export function FloatingAiOrb({ onClick, expanded = false }: { onClick: () => void; expanded?: boolean }) {
  return (
    <button
    ...
    <>
      <div className="relative z-10 border-b border-[rgba(196,115,122,0.1)] px-5 pb-4 pt-6 lg:flex-shrink-0">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button className="flex h-8 w-8 items-center justify-center rounded-xl bg-[rgba(28,22,20,0.04)] text-[var(--ink-faint)] transition-all hover:bg-[var(--petal-xlight)] hover:text-[var(--petal)]" onClick={onClose}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {chips.map((chip) => (
            <button key={chip} className="chip rounded-xl border border-[rgba(28,22,20,0.06)] bg-[rgba(28,22,20,0.05)] px-3 py-1.5 text-[11px] font-medium text-[var(--ink-muted)]">
              {chip}
            </button>
          ))}
        </div>
      ...
      <div className={`relative z-10 flex-1 overflow-y-auto px-5 py-5 ${compact ? 'max-h-[420px]' : ''}`}>
        <AiConversation />
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="my-1 flex items-center gap-3">
        ...
        <span className="rounded-full border border-[rgba(196,115,122,0.08)] bg-[var(--sand)] px-2.5 py-1 text-[10px] font-semibold text-[var(--ink-faint)]">今天 上午 9:12</span>
        <div className="flex-1 border-t border-[rgba(196,115,122,0.1)]" />
      </div>
...
function SidebarNavItem({ item }: { item: NavItem }) {
  return (
    <button className={`nav-item group relative flex items-center gap-3 overflow-hidden rounded-xl px-3 py-2.5 text-left text-[14px] ${item.active ? 'bg-[var(--petal-light)] text-[var(--petal)]' : 'text-[var(--ink-muted)] hover:bg-[var(--petal-xlight)]'}`}>
      ...
      <HomeIcon icon={item.icon} className={`relative h-[18px] w-[18px] ${item.active ? 'text-[var(--petal)]' : 'text-[var(--ink-faint)] group-hover:text-[var(--petal)]'}`} />
      ...
      {item.active ? (
        <svg viewBox="0 0 22 22" className="relative h-[22px] w-[22px]">
          ...
          <circle cx="11" cy="11" r="8" fill="none" stroke="var(--petal)" strokeWidth="2.5" strokeLinecap="round" strokeDasharray="50.27" strokeDashoffset="18" transform="rotate(-90 11 11)" />
        </svg>
      ) : null}
      {!item.active && item.badge ? (
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${item.label === '即将到来' ? 'bg-[rgba(232,168,124,0.15)] text-[#c48240]' : 'bg-[var(--petal-xlight)] text-[var(--petal)]'}`}>
          {item.badge}
        </span>
      ) : null}
    </button>
...
function StatTile({ icon, iconClassName, value, suffix, label }: { icon: ReactNode; iconClassName: string; value: string; suffix: string; label: string }) {
  return (
    <article className="stat-card glass flex items-center gap-3 rounded-xl border border-[rgba(196,115,122,0.1)] px-4 py-3">
      ...
      <div>
        <p className="leading-none text-[22px] font-semibold tracking-[-0.02em] text-[var(--ink)]">{value} <span className="text-[14px] font-normal text-[var(--ink-faint)]">{suffix}</span></p>
...
function TaskSection({ title, badge, tasks, completed = false, upcoming = false, badgeTone = 'petal' }: { title: string; badge?: string; tasks: Task[]; completed?: boolean; upcoming?: boolean; badgeTone?: 'petal' | 'teal' }) {
  return (
    <section>
      <div className="mb-3 flex items-center gap-3">
        <p className="text-[10px] font-semibold uppercase tracking-[0.07em] text-[var(--ink-faint)]">{title}</p>
        <div className="flex-1 border-t border-[rgba(196,115,122,0.1)]" />
        {badge ? <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${badgeTone === 'teal' ? 'bg-[var(--teal-xlight)] text-[var(--teal)]' : 'bg-[var(--petal-xlight)] text-[var(--petal)]'}`}>{badge}</span> : null}
      </div>
  ...
  if (completed) {
    return (
      <article className="task-card flex items-start gap-4 rounded-xl border border-[rgba(196,115,122,0.06)] bg-[rgba(255,255,255,0.45)] p-4 opacity-60 transition-opacity hover:opacity-80">
        ...
        <span className="flex flex-shrink-0 items-center gap-1 text-[12px] text-[var(--ink-faint)]"><CheckCircleIcon className="h-[13px] w-[13px]" />{task.doneAt}</span>
      </article>
    )
  }

  return (
    <article className={`task-card ${priorityClass(task.priority)} flex items-start gap-4 rounded-xl border p-4 ${upcoming ? 'glass border-[rgba(196,115,122,0.08)] hover:border-[rgba(196,115,122,0.2)]' : 'glass border-[rgba(196,115,122,0.1)] hover:border-[rgba(196,115,122,0.22)]'}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {!upcoming ? (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className="flex items-center gap-1.5 rounded-lg bg-[rgba(28,22,20,0.05)] px-2.5 py-1 text-[12px] text-[var(--ink-muted)]"><ClockIcon className="h-[13px] w-[13px]" />{task.time}</span>
            {task.tags.map((tag) => (
...
function CalendarIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M7 3v3M17 3v3M4 9h16" /><rect x="4" y="5" width="16" height="15" rx="3" /></svg>
}
function UpcomingIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M12 7v5l3 2" /><circle cx="12" cy="12" r="8" /></svg>
}
function SummaryIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><rect x="4" y="4" width="16" height="16" rx="3" /><path d="M8 9h8M8 13h8M8 17h5" /></svg>
}
function SlideshowIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M4 6h16v12H4z" /><path d="M10 9l5 3-5 3V9z" /></svg>
}
function FolderIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M4 7a2 2 0 012-2h4l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H6a2 2 0 01-2-2V7z" /></svg>
}
function PlusIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" {...props}><path d="M12 5v14M5 12h14" /></svg>
}
function TimerIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M10 2h4" /><path d="M12 14l3-2" /><circle cx="12" cy="14" r="7" /><path d="M12 7v7" /></svg>
}
function PlayIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="currentColor" {...props}><path d="M8 6.5v11l9-5.5-9-5.5z" /></svg>
}
function SettingsIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M12 3l1.6 2.8 3.2.7-.7 3.2L19 12l-2.9 1.3.7 3.2-3.2.7L12 20l-1.6-2.8-3.2-.7.7-3.2L5 12l2.9-1.3-.7-3.2 3.2-.7L12 3z" /><circle cx="12" cy="12" r="3" /></svg>
}
function MoreIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="currentColor" {...props}><circle cx="5" cy="12" r="1.8" /><circle cx="12" cy="12" r="1.8" /><circle cx="19" cy="12" r="1.8" /></svg>
}
function SunIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><circle cx="12" cy="12" r="4" /><path d="M12 2v2.5M12 19.5V22M4.9 4.9l1.8 1.8M17.3 17.3l1.8 1.8M2 12h2.5M19.5 12H22M4.9 19.1l1.8-1.8M17.3 6.7l1.8-1.8" /></svg>
}
function FilterIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M4 7h16M7 12h10M10 17h4" /></svg>
}
function CheckCircleIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><circle cx="12" cy="12" r="9" /><path d="M8.5 12.3l2.2 2.2 4.8-5.2" /></svg>
}
function SparkIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M12 3l1.9 5.1L19 10l-5.1 1.9L12 17l-1.9-5.1L5 10l5.1-1.9L12 3z" /></svg>
}
function ClockIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><circle cx="12" cy="12" r="8" /><path d="M12 8v4l3 2" /></svg>
}
function CheckIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" {...props}><path d="M5 12l4 4L19 6" /></svg>
}
function AttachmentIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M9 12.5l5.6-5.6a3 3 0 114.2 4.2l-7.1 7.1a5 5 0 11-7.1-7.1l7.1-7.1" /></svg>
}
function MicIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><rect x="9" y="3" width="6" height="11" rx="3" /><path d="M6 11a6 6 0 0012 0M12 17v4M9 21h6" /></svg>
}
function SendIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M4 11.5L20 4l-4.5 16-3.4-6.1L4 11.5z" /><path d="M12.1 13.9L20 4" /></svg>
}
function FlowerIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" {...props}><circle cx="12" cy="12" r="2.2" fill="currentColor" opacity="0.95" /><ellipse cx="12" cy="6.4" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" /><ellipse cx="17.2" cy="9.4" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" transform="rotate(60 17.2 9.4)" /><ellipse cx="17.1" cy="14.8" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" transform="rotate(120 17.1 14.8)" /><ellipse cx="12" cy="17.6" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" /><ellipse cx="6.8" cy="14.8" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" transform="rotate(60 6.8 14.8)" /><ellipse cx="6.8" cy="9.4" rx="2.1" ry="3.4" fill="currentColor" opacity="0.45" transform="rotate(120 6.8 9.4)" /></svg>
}
function ArrowLeftIcon(props: IconProps) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" {...props}><path d="M15 6l-6 6 6 6" /><path d="M9 12h10" /></svg>
}